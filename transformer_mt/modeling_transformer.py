...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_embeddings = nn.Embedding(num_embeddings=self.src_vocab_size, embedding_dim=self.hidden)
        self.decoder_embeddings = nn.Embedding(num_embeddings=self.tgt_vocab_size, embedding_dim=self.hidden)
        self.positional_emb = nn.Embedding(num_embeddings=self.tgt_vocab_size, embedding_dim=self.hidden)
        ...
        self.encoder_layers = nn.ModuleList([TransformerEncoderLayer(hidden=self.hidden, num_heads=self.num_heads,
                                                                     fcn_hidden=self.fcn_hidden,
                                                                     dropout=self.dropout_rate) for i in
                                             range(self.num_layers)])
        self.decoder_layers = nn.ModuleList([TransformerDecoderLayer(hidden=self.hidden, num_heads=self.num_heads,
                                                                     fcn_hidden=self.fcn_hidden,
                                                                     dropout=self.dropout_rate) for i in
                                             range(self.num_layers)])
